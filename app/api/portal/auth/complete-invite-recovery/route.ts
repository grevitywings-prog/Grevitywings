...
const invalidRecovery = "This invitation recovery link is invalid, expired or has already been used.";
...
export async function POST(request: NextRequest) {
  ...
  if (!tokenHash || tokenHash.length > 1024) return NextResponse.json({ error: invalidRecovery }, { status: 400 });
  ...
  if (verificationError || !verified.user) return applyCookies(NextResponse.json({ error: invalidRecovery }, { status: 400 }));
  ...
  if (!member || member.status !== "invited") {
    ...
    return applyCookies(NextResponse.json({ error: invalidRecovery }, { status: 403 }));
  }
  ...
  if (passwordError) return applyCookies(NextResponse.json({ error: "Your password could not be created." }, { status: 400 }));
  ...
  await writeAudit({
    ...
    action: "team_member_invitation_recovered",
    file_id: null,
    delivery_id: null,
